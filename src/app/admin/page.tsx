  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        if (!json.success || !json.data) throw new Error('Dashboard invalid payload');

        const { stats: st, genderData: gd, chartData: cd, upcomingEvents: ue, recentUsers: ru, recentApps: ra } = json.data;
        setStats(st);
        setGenderData(gd);
        setChartData(cd);
        ...
        const cachePayload = { stats: st, genderData: gd, chartData: cd, upcomingEvents: ue, recentUsers: ru, recentApps: ra, ts: Date.now() };
