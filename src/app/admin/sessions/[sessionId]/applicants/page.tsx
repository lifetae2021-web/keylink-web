    ...
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {(stats.maleConfirmed > (session?.maxMale || 0) || stats.femaleConfirmed > (session?.maxFemale || 0)) && (
          <div className="mb-6 p-4 rounded-2xl bg-rose-900/40 border-2 border-rose-500 flex items-center gap-4 animate-pulse">
            <XCircle className="text-rose-500 shrink-0" size={24} />
            <div className="flex-1">
              <p className="text-rose-100 font-black text-sm">정원을 초과했습니다. 확인 후 조정이 필요합니다.</p>
              <p className="text-rose-300 text-xs mt-1">
                현재 현황: 남 {stats.maleConfirmed}/{session?.maxMale || 0}명, 여 {stats.femaleConfirmed}/{session?.maxFemale || 0}명
              </p>
            </div>
          </div>
        )}
              ...
              value: (
                <div className="flex flex-col">
                  ...
                  <span className="text-[0.65rem] font-bold mt-1 opacity-70">
                    [선발 현황] 
                    <span className={stats.maleConfirmed > (session?.maxMale || 0) ? 'text-rose-400' : 'text-blue-400'}> 남 {stats.maleConfirmed}/{session?.maxMale || 0}</span>
                    <span className="mx-1">/</span>
                    <span className={stats.femaleConfirmed > (session?.maxFemale || 0) ? 'text-rose-400' : 'text-pink-400'}> 여 {stats.femaleConfirmed}/{session?.maxFemale || 0}</span>
                  </span>
                </div>
              ), 
            ...
            <table className="w-full">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  // v8.1.6: 초과 인원 판별 로직 (확정자 중 정원 이후에 확정된 유저 강조)
