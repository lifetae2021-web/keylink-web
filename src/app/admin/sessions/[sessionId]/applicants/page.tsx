...
import { query, collection, where, onSnapshot } from 'firebase/firestore'; // v8.1.7
...
export default function ApplicantsPage() {
  ...
  useEffect(() => {
    const unsub = onAuthStateChanged(auth, async (user) => {
      ...
      if (snap.exists() && snap.data().role === 'admin') {
        setIsAdmin(true);
        // loadData(); // onSnapshot으로 대체 (v8.1.7)
      } else {
        router.push('/');
      }
    });
  ...
  useEffect(() => {
    ...
    const appsUnsub = onSnapshot(q, (snap) => {
  ...
  const loadData = async () => {
    // onSnapshot 사용으로 더 이상 명시적인 초기 로드 불필요하나,
    // 필요 시 강제 새로고침 용도로 남겨둘 수 있음.
    setLoading(true);
    try {
      const apps = await getApplicationsBySession(sessionId);
  ...
  const stats = {
    ...
    // v8.1.7: 'confirmed' 상태만 정원에 포함 (selected = 입금 대기, 자리 미확정)
  ...
  return (
    <div className="min-h-screen bg-gray-950 text-white p-6">
      <div className="max-w-7xl mx-auto">
        ...
        {/* v8.1.7: 초과 인원 경고 배너 */}
        {(stats.maleConfirmed > (session?.maxMale || 0) || stats.femaleConfirmed > (session?.maxFemale || 0)) && (
          <div className="mb-6 p-5 rounded-2xl bg-rose-600 border-2 border-rose-400 flex items-center gap-4 animate-bounce">
            ...
            <div className="flex-1">
              ...
              <p className="text-rose-100 text-sm mt-1 font-bold">
                참여자 상태를 조정하여 정원을 맞추거나, 행사 설정에서 정원을 늘려주세요.
              </p>
            </div>
          </div>
        )}
            ...
            { 
              ...
              value: (
                <div className="flex flex-col">
                  <span className={stats.confirmed > ((session?.maxMale || 0) + (session?.maxFemale || 0)) ? 'animate-pulse' : ''}>
                    {stats.confirmed}
                  </span>
                  <span className="text-[0.65rem] font-bold mt-1 opacity-70">
                    [선발 현황] 
                    <span className={stats.maleConfirmed > (session?.maxMale || 0) ? 'text-rose-500 animate-pulse font-black scale-110 inline-block' : 'text-blue-400'}> 남 {stats.maleConfirmed}/{session?.maxMale || 0}</span>
                    <span className="mx-1">/</span>
                    <span className={stats.femaleConfirmed > (session?.maxFemale || 0) ? 'text-rose-500 animate-pulse font-black scale-110 inline-block' : 'text-pink-400'}> 여 {stats.femaleConfirmed}/{session?.maxFemale || 0}</span>
                  </span>
                </div>
              ), 
              color: stats.confirmed > ((session?.maxMale || 0) + (session?.maxFemale || 0)) ? '#F43F5E' : '#10B981' 
            },
            ...
            <table className="w-full">
              ...
              <tbody>
                {filtered.length === 0 ? (
                  ...
                  const maxCount = isMale ? (session?.maxMale || 0) : (session?.maxFemale || 0);
                  const currentCount = isMale ? stats.maleConfirmed : stats.femaleConfirmed;
                  const isFull = currentCount >= maxCount;

                  // v8.1.7: 초과 인원 판별 로직 (확정자 중 정원 이후에 확정된 유저 강조)
                  let isOverCapacity = false;
                  if (app.status === 'confirmed') {
                    const confirmedList = applications
                      .filter(a => a.status === 'confirmed' && a.gender === app.gender)
                      .sort((a, b) => (a.updatedAt?.getTime() || 0) - (b.updatedAt?.getTime() || 0));
                    const order = confirmedList.findIndex(a => a.id === app.id);
                    if (order >= maxCount) isOverCapacity = true;
                  }

                  return (
                  <tr key={app.id} className={`border-b border-gray-800/50 hover:bg-gray-800/30 transition-colors ${isOverCapacity ? 'bg-rose-950/30' : ''}`}>
                    <td className="p-4">
                      {app.status === 'applied' && (
                        <input
                          type="checkbox"
                    ...
                    <td className="p-4">
                        <span
                          className={`px-3 py-1 rounded-lg text-xs font-bold ${isOverCapacity ? 'bg-rose-500 text-white animate-pulse' : ''}`}
                          style={isOverCapacity ? {} : { background: `${STATUS_COLORS[app.status]}20`, color: STATUS_COLORS[app.status] }}
                        >
                          {isOverCapacity ? '정원 초과 확정' : STATUS_LABELS[app.status]}
                        </span>
                      </td>
                    <td className="p-4">
                        ...
                        {app.status === 'applied' && (
                          <div className="flex items-center gap-2">
                             <button
                               ...
                               className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all disabled:opacity-30 ${isFull ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-blue-600/20 text-blue-300 hover:bg-blue-600/40'}`}
                               title={isFull ? '정원이 가득 찼습니다' : ''}
                             >
                               선발확정
                             </button>
                             <button
                               ...
                               disabled={actionLoading === app.id || isFull}
                               className={`px-3 py-1.5 rounded-lg text-xs font-bold transition-all disabled:opacity-30 ${isFull ? 'bg-gray-800 text-gray-500 cursor-not-allowed' : 'bg-purple-600/20 text-purple-300 hover:bg-purple-600/40'}`}
                             >
                               선발
                             </button>
                          </div>
