...
/**
 * Firestore Timestamp 또는 Date 객체를 안전하게 JS Date로 변환하는 Type Guard 헬퍼.
 * `as any` 를 남용하지 않고 런타임에서 안전하게 toDate()를 호출합니다.
 */
...
export default function EventsPage() {
  ...
  // v14.1.0: 선발 전 경고 모달 상태
  const [preSelectWarningOpen, setPreSelectWarningOpen] = useState(false);
  const [preSelectWarnings, setPreSelectWarnings] = useState<{ type: 'overlap' | 'acquaintance'; message: string }[]>([]);
  const [preSelectPendingApp, setPreSelectPendingApp] = useState<Application | null>(null);
  const [preSelectPendingLabel, setPreSelectPendingLabel] = useState<string | undefined>(undefined);
  const [preSelectPendingAction, setPreSelectPendingAction] = useState<'select' | 'confirm'>('select');
  ...
  // v11.1.2: UI 통계 표시용 (더미 계정 및 닼템 제외)
  ...
  const handleBulkSMSConfirm = async (message: string, updatedPrice?: number, finalTargets?: any[], scheduledDate?: string) => {
    ...
    const sentAppIds = targetsToSend.filter((t: { appId?: string }) => t.appId).map((t: { appId?: string }) => t.appId!);
    try {
      ...
      if (sentAppIds.length > 0) {
        setApplicants((prev) =>
          prev.map((a) =>
            sentAppIds.includes(a.id) ? { ...a, secondSmsSentAt: sentAt } : a
          )
        );
      }
    } catch (e) {
  ...
  const handleBulk2ndSMSClick = () => {
    ...
    const eventDate = toJsDate(session?.eventDate);
...
${chatLink}`;
  ...
  // v14.1.0: 선발 전 경고 체크 (지인회피 + 중복만남)
  const checkPreSelectWarnings = async (app: Application): Promise<{ type: 'overlap' | 'acquaintance'; message: string }[]> => {
    const warnings: { type: 'overlap' | 'acquaintance'; message: string }[] = [];

    // 1-1. 신청자 본인이 지인회피를 작성한 경우 체크 (application + userMap 둘 다 확인)
    const userDoc = userMap[app.userId];
    const appAvoidList: any[] = (app.avoidList || []).filter(
      (e: any) => (e.name && e.name.trim()) || (e.birthYear && e.birthYear.trim()) || (e.workplace && e.workplace.trim())
    );
    const userDocData = userDoc as Record<string, any> | undefined;
    const userAvoidList: any[] = ((userDocData?.avoidList) || []).filter(
      (e: any) => (e.name && e.name.trim()) || (e.birthYear && e.birthYear.trim()) || (e.workplace && e.workplace.trim())
    );
    const effectiveAvoidList = appAvoidList.length > 0 ? appAvoidList : userAvoidList;
    const legacyAvoid = (app.avoidAcquaintance && app.avoidAcquaintance.trim())
      || (userDocData?.avoidAcquaintance && String(userDocData.avoidAcquaintance).trim());

    const hasAvoidInfo = effectiveAvoidList.length > 0 || !!legacyAvoid;
    if (hasAvoidInfo) {
      const avoidDetail = effectiveAvoidList.length > 0
        ? effectiveAvoidList.map((e: any) => {
            const parts = [];
            if (e.name && e.name.trim()) parts.push(`이름: ${e.name}`);
            if (e.birthYear && e.birthYear.trim()) parts.push(`년생: ${e.birthYear}`);
            if (e.workplace && e.workplace.trim()) parts.push(`직장/거주지: ${e.workplace}`);
            return parts.join(' / ');
          }).join('\n')
        : (legacyAvoid || '');
      warnings.push({
        type: 'acquaintance',
        message: `이 신청자는 지인 회피 요청을 한 기록이 있습니다.\n지인일 것으로 의심되는 분이 현재 참가자 명단에 있을 수 있으니 한 번 더 확인해 주세요.\n\n📋 회피 요청 내용:\n${avoidDetail}`,
      });
    }

    // 1-2. 다른 확정 참가자가 이 신청자를 지인회피 대상으로 지정했는지 체크 (역방향)
    const confirmedOthers = applicants.filter(a => (a.status === 'confirmed' || a.status === 'selected') && a.id !== app.id);
    const avoidingNames: string[] = [];
    
    for (const otherApp of confirmedOthers) {
      const otherUserDoc = userMap[otherApp.userId];
      const otherAppAvoid = (otherApp.avoidList || []).filter((e: any) => e.name || e.birthYear || e.workplace);
      const otherUserDocData = otherUserDoc as Record<string, any> | undefined;
      const otherUserAvoid = ((otherUserDocData?.avoidList) || []).filter((e: any) => e.name || e.birthYear || e.workplace);
      const otherEffectiveAvoid = otherAppAvoid.length > 0 ? otherAppAvoid : otherUserAvoid;
      const otherLegacy = (otherApp.avoidAcquaintance || '') + ' ' + (otherUserDocData?.avoidAcquaintance || '');

      let isTargeted = false;
      if (app.name) {
        // 이름이 부분 일치라도 하면 의심으로 간주
        if (otherEffectiveAvoid.some((e: any) => e.name && e.name.includes(app.name))) isTargeted = true;
        if (otherLegacy.includes(app.name)) isTargeted = true;
      }
      
      if (isTargeted) {
        avoidingNames.push(otherApp.name || '참가자');
      }
    }
    
    if (avoidingNames.length > 0) {
      warnings.push({
        type: 'acquaintance',
        message: `현재 참가자 명단에 있는 ${avoidingNames.join(', ')} 님이 이 신청자(${app.name})를 지인회피 대상으로 지정했을 가능성이 높습니다.\n양측의 인적사항(나이, 직장 등)을 한 번 더 확인해 주세요.`,
      });
    }

    // 2. 중복만남 기록 체크 (현재 기수 확정/선발 참가자 vs 신청자 과거 기수)
    try {
      const session = active;
      if (session) {
        const oppositeGender = app.gender === 'male' ? 'female' : 'male';
        // confirmed + selected 상태 모두 포함
        const confirmedOpposite = applicants.filter(
          (a) => a.gender === oppositeGender && (a.status === 'confirmed' || a.status === 'selected')
        );

        if (confirmedOpposite.length > 0) {
          // 신청자의 과거 확정(confirmed) 또는 선발(selected) 기수 조회
          const [appPastConfirmedSnap, appPastSelectedSnap] = await Promise.all([
            getDocs(query(collection(db, 'applications'), where('userId', '==', app.userId), where('status', '==', 'confirmed'))),
            getDocs(query(collection(db, 'applications'), where('userId', '==', app.userId), where('status', '==', 'selected'))),
          ]);
          const appPastSessionIds = [
            ...appPastConfirmedSnap.docs.map((d) => d.data().sessionId as string),
            ...appPastSelectedSnap.docs.map((d) => d.data().sessionId as string),
          ].filter((sid) => sid !== session.id);

          if (appPastSessionIds.length > 0) {
            const overlapNames: string[] = [];
            for (const opp of confirmedOpposite) {
              const [oppConfirmedSnap, oppSelectedSnap] = await Promise.all([
                getDocs(query(collection(db, 'applications'), where('userId', '==', opp.userId), where('status', '==', 'confirmed'))),
                getDocs(query(collection(db, 'applications'), where('userId', '==', opp.userId), where('status', '==', 'selected'))),
              ]);
              const oppPastSessionIds = [
                ...oppConfirmedSnap.docs.map((d) => d.data().sessionId as string),
                ...oppSelectedSnap.docs.map((d) => d.data().sessionId as string),
              ].filter((sid) => sid !== session.id);
              const common = appPastSessionIds.filter((sid) => oppPastSessionIds.includes(sid));
              if (common.length > 0) {
                overlapNames.push(opp.name || '참가자');
              }
            }
            if (overlapNames.length > 0) {
              warnings.push({
                type: 'overlap',
                message: `이 신청자는 현재 기수에 이미 선발된 ${overlapNames.join(', ')}님과 중복 만남 기록이 있습니다.\n같은 기수에 배정 시 과거에 만났던 분과 재회할 수 있습니다.`,
              });
            }
          }
        }
      ...
      console.error('선발 전 중복만남 체크 오류:', e);
    }

    return warnings;
  };

  const handleWaitlistSelect = async (app: Application, confirmLabel?: string) => {
    // v14.1.0: 선발 버튼 클릭 시 경고 먼저 체크
    const warnings = await checkPreSelectWarnings(app);
    if (warnings.length > 0) {
      setPreSelectWarnings(warnings);
      setPreSelectPendingApp(app);
      setPreSelectPendingLabel(confirmLabel);
      setPreSelectPendingAction('select');
      setPreSelectWarningOpen(true);
      return;
    }
    handleWaitlistSelectProceed(app, confirmLabel);
  };

  const handleWaitlistSelectProceed = (app: Application, confirmLabel?: string) => {
    const session = active;
    if (!session) return toast.error("세션 정보를 찾을 수 없습니다.");
    const user = userMap[app.userId] || {};

    const eventTime = toJsDate(session.eventDate);
...
좋은 인연 만날 수 있도록 키링크가 끝까지 책임질게요🥰`;
  ...
  const handleWaitlistConfirm = async (app: Application) => {
    // 선발확정 버튼도 동일한 경고 체크 적용
    const warnings = await checkPreSelectWarnings(app);
    if (warnings.length > 0) {
      setPreSelectWarnings(warnings);
      setPreSelectPendingApp(app);
      setPreSelectPendingLabel(undefined);
      setPreSelectPendingAction('confirm');
      setPreSelectWarningOpen(true);
      return;
    }
    handleWaitlistConfirmProceed(app);
  };

  const handleWaitlistConfirmProceed = (app: Application) => {
    const session = active;
    if (!session) return toast.error("세션 정보를 찾을 수 없습니다.");
    const user = userMap[app.userId] || {};
...
당일 현장에서 뵙겠습니다! 좋은 인연 만나시길 바랍니다 :)`;
  ...
  const handleSetAttendanceStatus = async (app: Application, status: 'present' | 'late' | 'no-show' | 'excused' | 'none') => {
    try {
    ...
    try {
      ...
      if (!existing.empty) {
        ...
        if (existingData.isDarkTemplar === true) {
          return toast.error('이미 닼템로 참여 중입니다.');
        }
        
        const proceed = window.confirm('이미 이 기수에 일반 신청서가 존재합니다. 닼템(무제한 정원 외)로 전환하여 참가 확정하시겠습니까?');
        if (!proceed) {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || '닼템 참여에 실패했습니다.');
      toast.success('🌑 닼템로 참여가 등록되었습니다.');
    } catch (e: any) {
      toast.error(e.message || '오류가 발생했습니다.');
    } finally {
      setIsDarkTemplarJoining(false);
  ...
  const handleDarkTemplarLeave = async (sessionId: string) => {
    ...
    const proceed = window.confirm('닼템 참여를 해제하시겠습니까?');
    if (!proceed) return;

    setIsDarkTemplarJoining(true);
    try {
      // 해당 기수에 닼템로 등록된 본인의 신청서 ID 찾기
      const { collection: col, query: q, where: wh, getDocs: gd } = await import('firebase/firestore');
      const snap = await gd(q(col(db, 'applications'), wh('userId', '==', user.uid), wh('sessionId', '==', sessionId), wh('isDarkTemplar', '==', true)));
      
      if (snap.empty) {
        throw new Error('닼템 신청 내역을 찾을 수 없습니다.');
      }
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error || '닼템 해제에 실패했습니다.');
      toast.success('🌑 닼템 참여가 해제되었습니다.');
    } catch (e: any) {
      toast.error(e.message || '오류가 발생했습니다.');
    } finally {
      setIsDarkTemplarJoining(false);
...
카카오프렌즈 익명으로 입장해주시면 됩니다 ! 내일 오픈채팅으로 진행과정에 대해 설명드리니 지금 바로 입장부탁드립니다 :)`;
  ...
  const handleCancelSession = async (session: Session) => {
    ...
    try {
      ...
      if (session.eventDate) {
        try {
          const dateObj = toJsDate(session.eventDate);
  ...
  return (
    ...
    femaleGroupPrice: globalSettings?.femaleGroupPrice ? globalSettings.femaleGroupPrice.toLocaleString() : "24,000",
            ...
            {sessions.filter((ev) => {
              ...
              return (
                ...
                >
                  ...
                  {ev.targetMaleAge && (
                    <p className={`flex items-center gap-1 text-[0.68rem] font-semibold mb-2 ${ev.isCustomCuration ? 'text-[#FF6F61]' : 'text-blue-400'}`}>
                      {!ev.isCustomCuration && <span className="text-[0.6rem]">👨</span>}
                      {ev.isCustomCuration ? '여성 우선 선발' : ev.targetMaleAge}
                    </p>
                  )}
                  <div
              ...
              <div className={card}>
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 px-5 sm:px-7 py-5 border-b border-slate-100 bg-white">
                  ...
                  <div className="flex items-center gap-1.5 overflow-x-auto max-w-full no-scrollbar pb-1.5 sm:pb-0 scroll-smooth shrink-0 w-full sm:w-auto sm:overflow-x-visible">
                    ...
                    {/* 🌑 닼템 참여/해제 토글 버튼 - super_admin에게만 표시 */}
                    {isSuperAdmin && (() => {
                      ...
                      return (
                        <button
                          ...
                          title={alreadyJoined ? '클릭 시 닼템 참여 해제' : '닼템로 참여'}
                        >
                              ...
                              return (
                                <div className="flex items-center flex-wrap gap-2 bg-blue-50 px-3 py-1.5 rounded-lg border border-blue-100">
                                  ...
                                  {codes.map(c => {
                                    ...
                                    const servedCount = totalForDrink.filter(p => p.drinkServed).length;
                                            ...
                                            <div className="flex items-center justify-between gap-2">
                                              <div className="flex items-center gap-2 min-w-0">
                                                ...
                                                <div className="flex items-center gap-1.5 flex-nowrap min-w-0 overflow-hidden">
                                                  ...
                                                  {app.isDarkTemplar && (
                                                    <span className="text-[0.6rem] font-bold px-1.5 py-0.5 rounded bg-violet-100 text-violet-700 border border-violet-300 shrink-0" title="닼템 - 매칭 제외">🌑다크</span>
                                                  )}
                                                  {(() => {
                                            ...
                                            <div className="flex items-center gap-3">
                                              ...
                                              <div className="flex-1 min-w-0">
                                                <div className="flex items-center justify-between sm:justify-start gap-2 mb-0.5">
                                                  ...
                                                  {app.isDarkTemplar && (
                                                    <span className="text-[0.6rem] font-bold px-1.5 py-0.5 rounded bg-violet-200 text-violet-800 border border-violet-400 shrink-0" title="닼템 - 매칭 제외">
                                                      🌑 닼템
                                                    </span>
                                                  )}
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={q.key}>
                      ...
                      <input
                        type="text"
                        value={(configFormData as Record<string, unknown>)[q.key] as string || ""}
                        onChange={(e) =>
                          setConfigFormData({
                            ...configFormData,
                    ...
                    >
                      <input
                        ...
                        onChange={() =>
                          setConfigFormData({
                            ...configFormData,
                            resultVisibility: opt.id as 'all' | 'mutual',
                          })
                        }
                ...
                return (
                  <div className="space-y-2.5">
                    {targetParticipants.map(p => {
                      const isServed = p.drinkServed === true;
      ...
      {preSelectWarningOpen && preSelectPendingApp && (
        <div
          onClick={() => { setPreSelectWarningOpen(false); setPreSelectPendingApp(null); }}
        ...
        >
          ...
          >
            ...
            <div className="flex items-center gap-3 px-6 py-4 bg-rose-50 border-b border-rose-100">
              ...
              <div>
                <h3 className="text-sm font-extrabold text-rose-700">선발 전 확인이 필요합니다</h3>
                <p className="text-xs text-rose-400 mt-0.5">{preSelectPendingApp.name || '신청자'} 님 · {preSelectPendingApp.gender === 'male' ? '남성' : '여성'}</p>
              </div>
            </div>

            {/* Warning Items */}
            <div className="px-5 py-4 flex flex-col gap-3 max-h-[60vh] overflow-y-auto">
              {preSelectWarnings.map((w, i) => (
                <div
                  key={i}
                  className={`rounded-xl p-4 border ${w.type === 'overlap'
                    ? 'bg-rose-50 border-rose-200'
                    : 'bg-amber-50 border-amber-200'
                  }`}
                >
                  <p className={`text-xs font-extrabold mb-1.5 flex items-center gap-1.5 ${w.type === 'overlap' ? 'text-rose-600' : 'text-amber-600'}`}>
                    {w.type === 'overlap' ? '🔴 중복 만남 기록이 있습니다' : '🟡 지인일 것으로 의심됩니다'}
                  </p>
                  <p className="text-[0.75rem] text-slate-600 font-medium leading-relaxed whitespace-pre-wrap">{w.message}</p>
                </div>
              ))}
            </div>
            ...
            <div className="flex gap-2 px-5 pb-5 pt-2">
              <button
                onClick={() => {
                  setPreSelectWarningOpen(false);
                  setPreSelectPendingApp(null);
                }}
                className="flex-1 py-3 rounded-xl text-sm font-extrabold bg-slate-100 text-slate-600 hover:bg-slate-200 transition-colors"
              >
                취소
              </button>
              <button
                onClick={() => {
                  const app = preSelectPendingApp;
                  const label = preSelectPendingLabel;
                  const action = preSelectPendingAction;
                  setPreSelectWarningOpen(false);
                  setPreSelectPendingApp(null);
                  setPreSelectPendingLabel(undefined);
                  if (app) {
                    if (action === 'confirm') {
                      ...
                      handleWaitlistSelectProceed(app, label);
                    }
                  }
                }}
                className="flex-1 py-3 rounded-xl text-sm font-extrabold bg-rose-500 text-white hover:bg-rose-600 transition-colors shadow-sm"
              >
                그래도 {preSelectPendingAction === 'confirm' ? '확정' : '선발'}
              </button>
            </div>
          </div>
        </div>
            ...
            >
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            setFormData(prev => ({
                              ...
                              ageStart: e.target.checked ? "" : prev.ageStart,
                              ageEnd: e.target.checked ? "" : prev.ageEnd,
                            }));
                          }}
                    ...
                    {!formData.isCustomCuration ? (
                      <div className="flex items-center gap-2">
                        <input
                          ...
                          onChange={(e) => {
                            const val = e.target.value.replace(/[^0-9]/g, "");
                            setFormData(prev => ({ ...prev, ageStart: val }));
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            const val = e.target.value.replace(/[^0-9]/g, "");
                            setFormData(prev => ({ ...prev, ageEnd: val }));
                          }}
                          className="w-16 h-11 text-center rounded-xl border border-slate-200 bg-white text-slate-800 font-bold focus:ring-2 focus:ring-[#FF6F61]/20 focus:border-[#FF6F61] outline-none transition-all"
                        />
              ...
              <div className="mb-6">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setFormData(prev => ({ ...prev, openChatLink: e.target.value }))
                  }
                  className="w-full h-11 px-4 rounded-xl border border-slate-200 bg-white text-slate-800 font-semibold focus:ring-2 focus:ring-[#FF6F61]/20 focus:border-[#FF6F61] outline-none transition-all"
                />
              ...
              <div className="mb-6">
                <label className="block text-xs font-bold text-slate-500 mb-1.5 uppercase tracking-wide">
                  행사 테마 (특집) <span className="text-slate-400 font-normal ml-1">선택사항</span>
                </label>
                <input
                  type="text"
                  placeholder="예: 공기업/대기업, 전문직, 20대 후반 등 (빈칸 시 일반 기수)"
                ...
                />
                <p className="text-[10px] text-slate-400 font-medium mt-1.5 ml-1">
                  * 입력 시 사용자 안내 화면에 테마가 눈에 띄게 표시됩니다.
                </p>
              </div>
