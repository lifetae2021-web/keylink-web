...
${chatLink}`;
  ...
  const checkPreSelectWarnings = async (app: Application): Promise<{ type: 'overlap' | 'acquaintance'; message: string }[]> => {
    ...
    // 2. 중복만남 기록 체크 (현재 기수 확정 참가자 vs 신청자 과거 기수)
    try {
      const session = active;
      if (session) {
        ...
        if (confirmedOpposite.length > 0) {
          // 신청자의 과거 확정 기수 조회
          const appPastSnap = await getDocs(
            query(collection(db, 'applications'), where('userId', '==', app.userId), where('status', '==', 'confirmed'))
          );
          const appPastSessionIds = appPastSnap.docs
            .map((d) => d.data().sessionId as string)
            .filter((sid) => sid !== session.id);

          if (appPastSessionIds.length > 0) {
            // 반대 성별 확정 참가자들의 과거 기수와 겹치는지 확인
            const overlapNames: string[] = [];
            for (const opp of confirmedOpposite) {
              const oppPastSnap = await getDocs(
                query(collection(db, 'applications'), where('userId', '==', opp.userId), where('status', '==', 'confirmed'))
              );
              const oppPastSessionIds = oppPastSnap.docs
                .map((d) => d.data().sessionId as string)
                .filter((sid) => sid !== session.id);
            ...
            <div className="flex gap-2 px-5 pb-5 pt-2">
              ...
              <button
                onClick={() => {
                  ...
                  if (app) handleWaitlistSelectProceed(app, label);
                }}
