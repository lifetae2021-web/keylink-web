...
export default function MyPage() {
  ...
  useEffect(() => {
    const unsubscribe = onAuthStateChanged(auth, async (currentUser) => {
      ...
      // v8.1.2: 모든 신청서 실시간 구독 및 관련 세션 실시간 추적
...
// ─────────────────────────────────────────────────────────────────────────────
// v8.1.2 ApplicationStatusSection — 다중 카드 렌더링 지원
// ─────────────────────────────────────────────────────────────────────────────
...
function ApplicationStatusBlock({ application, session, userId, hasVoted }: StatusBlockProps) {
  ...
  if (application.status === 'confirmed') {
    // v8.1.2: KST 기준 행사 당일 00:00부터 투표 활성화
...
// v8.1.2 ReceivedHeartsFeed — 나를 선택한 이성 정보 및 맞호감 표시
// ─────────────────────────────────────────────────────────────────────────────
