...
async function run() {
  ...
  if (choiDoc && baekDoc) {
    ...
    await db.collection('applications').doc(choiId).update({
      companionIds: admin.firestore.FieldValue.arrayUnion(baekId),
      companionNames: admin.firestore.FieldValue.arrayUnion('백선미')
    });
    
    await db.collection('applications').doc(baekId).update({
      companionIds: admin.firestore.FieldValue.arrayUnion(choiId),
      companionNames: admin.firestore.FieldValue.arrayUnion('최은진')
    });
    
    console.log(`Successfully linked ${choiId} and ${baekId} for session ${sessionId134}`);
  } else {
