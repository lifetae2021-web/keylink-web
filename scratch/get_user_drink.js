...
async function checkGitak() {
  ...
  if (snap.empty) {
    console.log('No application found for 이기탁 in this session.');
    
    // Search by name only
    console.log('Searching by name only across all sessions...');
    const nameSnap = await db.collection('applications').where('name', '==', name).get();
    if (nameSnap.empty) {
      console.log('No application found for name 이기탁 at all.');
    } else {
